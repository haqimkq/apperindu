...
class Template_rekomendasi extends BaseController
{
    ...
    private $path = 'Master/template_rekomendasi';
    ...
    public function __construct()
    {
        ...
        $this->model =  new M_template_rekomendasi($this->request);
    ...
    public function get_data()
    {
        ...
        foreach ($lists as $l) {
            ...
            $row[] = '<div class="text-wrap"><a href="' . base_url('doc/template/' . $l['tblskizin_rekomtemplate']) . '">' . $l['tblskizin_rekomtemplate'] . '</a></div>';
    ...
    public function form()
    {
        $rules = [
            ...
            'tblskizin_rekomtemplate' => [
                'rules' => 'uploaded[tblskizin_rekomtemplate]|ext_in[tblskizin_rekomtemplate,rtf]',
                'errors' => [
        ...
        if (!$this->validate($rules)) {
            ...
            $res = array('status' => false, 'msg' => $this->validator->getError('tblskizin_rekomtemplate'));
        ...
        $file = $this->request->getFile('tblskizin_rekomtemplate');
        $file->move('doc/template');
        $d['tblskizin_rekomtemplate'] = $file->getName();
    ...
    public function update_template()
    {
        ...
        $tblskizin_rekomtemplate_old = $this->request->getPost('tblskizin_rekomtemplate_old');
        $file = $this->request->getFile('tblskizin_rekomtemplate');
        ...
        if (!$this->validate($rules)) {
            session()->setFlashdata('error', $this->validator->getError('tblskizin_rekomtemplate'));
            return redirect()->to('/' . $this->url . '/testing/' . $id);
        }


        if (file_exists('doc/template/' . $tblskizin_rekomtemplate_old)) {
            // unlink('doc/template/' . $tblskizin_rekomtemplate_old);
        }
    ...
    public function form_update()
    {
        $rules = [
            ...
            'tblskizin_rekomtemplate' => [
                'rules' => 'ext_in[tblskizin_rekomtemplate,rtf]',
                'errors' => [
                    'ext_in' => 'Harap upload file template dengan format rtf'
        ...
        if ($file->getError() == 4) {
            $d['tblskizin_rekomtemplate'] = $this->request->getPost('tblskizin_rekomtemplate_old');
        } else {

            if (file_exists('doc/template/' . $this->request->getPost('tblskizin_rekomtemplate_old'))) {
                // unlink('doc/template/' . $this->request->getPost('tblskizin_rekomtemplate_old'));
            }
    ...
    public function testing($id)
    {
        ...
        $variable['bsre_logo'] = '';
        $variable['TTD'] = $qr;
        $variable['nama_kadis'] = replace_variable('nama_kadis');
        $variable['pangkat_kadis'] = replace_variable('pangkat_kadis');
        $variable['nip_kadis'] = replace_variable('nip_kadis');
        ...
        $template = 'doc/template/' . $row['tblskizin_rekomtemplate'];
        ...
        $data['path'] = $this->path;
        $data['request'] = $this->request;
        $data['template'] =  $row['tblskizin_rekomtemplate'];
